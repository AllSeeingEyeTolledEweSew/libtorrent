...
class ParseMagnetTest(unittest.TestCase):
    ...
    @unittest.skip("we should raise an explicit error for this")
    def test_parse_dict_non_unicode_dn(self) -> None:
        uri = f"magnet:?xt=urn:btih:{self.info_hash_sha1}&dn=%xff"
        with self.assertRaises(RuntimeError):
            with warnings.catch_warnings():
                lt.parse_magnet_uri_dict(uri)
...
class AddMagnetUriTest(unittest.TestCase):
    ...
    def test_deprecation(self) -> None:
        uri = f"magnet:?xt=urn:btih:{self.info_hash_sha1}"
        with self.assertWarns(DeprecationWarning):
            lt.add_magnet_uri(self.session, uri, {"save_path": self.dir.name})
